...
public class Function {
    ...
    public List<List<String>> filterRow(String[] titles, List<List<String>> data, String columnTitle, String sign, Integer value) {
        int index = coulmnTranslator(columnTitle, titles);
        if( sign.equals("=")) {
            return data.stream()
                    .filter(n -> Integer.valueOf(n.get(index)).equals(value) )
                    .collect(Collectors.toList());
        }
        if( sign.equals( ">")){
            return data.stream()
